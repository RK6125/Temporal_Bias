...
class BiasModelTester:
    ...
    def _load_models(self):
        ...
        try:
            ...
            models['bert'] = pipeline("sentiment-analysis", #type: ignore
                                    model="cardiffnlp/twitter-roberta-base-sentiment-latest",
                                    return_all_scores=True) #type: ignore
            self.logger.info(" DistilBERT sentiment model:")
            models['distilbert'] = pipeline("sentiment-analysis", #type: ignore
                                           model="distilbert-base-uncased-finetuned-sst-2-english",
                                           return_all_scores=True) #type: ignore
            self.logger.info("VADER-style model:")
            models['vader_style'] = pipeline("sentiment-analysis", #type: ignore
    ...
    def get_sentiment_scores(self, text, model_name):
     try:
    ...
    def analyze_single_text(self, text):
        ...
        for model_name in self.models.keys():
            results[model_name] = self.get_sentiment_scores(text, model_name)
        return results
    ...
    def extract_demographic_context(self, row):
    ...
    def run_bias_analysis(self, df):
        ...
        for idx, row in df.iterrows():
            if idx % 100 == 0:
                self.logger.info(f"Processing {idx}/{len(df)} samples...")
            
            text = row['text']
            context = self.extract_demographic_context(row)
    ...
    def calculate_bias_metrics(self, results_df):
        ...
        for model_name in self.models.keys():
            ...
            gender_prof_bias = self._calculate_intersection_bias(
                results_df, 'gender', 'profession', sentiment_col, score_col
        )
            model_metrics['gender_profession'] = gender_prof_bias
            race_auth_bias = self._calculate_intersection_bias(
                results_df, 'race', 'authority', sentiment_col, score_col
        )
            model_metrics['race_authority'] = race_auth_bias
        
            temporal_bias = self._calculate_temporal_bias(
                results_df, sentiment_col, score_col
        )
            model_metrics['temporal_changes'] = temporal_bias
        
            temporal_by_community = self._calculate_temporal_bias_by_community(
                results_df, sentiment_col, score_col
        )
    ...
    def _calculate_intersection_bias(self, df, demo_col, context_col, sentiment_col, score_col):
        bias_results = {}  
    ...
    def _calculate_temporal_bias(self, df, sentiment_col, score_col):
    ...
    def _calculate_temporal_bias_by_community(self, df, sentiment_col, score_col):
        community_temporal = {}
    ...
    def create_bias_report(self, results_df, bias_metrics):
        ...
        report = {
            ...
            'key_findings': self._extract_key_findings(bias_metrics),
            'recommendations': self._generate_recommendations(bias_metrics)
        }
        
        return report
    
    def _extract_key_findings(self, bias_metrics):
        findings = []
    ...
    def _generate_recommendations(self, bias_metrics):
        return [
            "Compare bias patterns across different models to identify consistent vs. model-specific biases",
            "Focus on intersections with large sample sizes for reliable statistics",
            "Investigate temporal changes in bias patterns across the four time periods",
            "Consider additional bias mitigation techniques for models showing extreme bias ratios",
            "Validate findings with human evaluation on high-bias samples"
        ]
    ...
    def run_complete_analysis(self):
        ...
        report = self.create_bias_report(results_df, bias_metrics)
...
def main():
    ...
    results_df, bias_metrics, report = tester.run_complete_analysis()
